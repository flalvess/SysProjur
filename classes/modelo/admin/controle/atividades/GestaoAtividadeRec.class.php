...
class GestaoAtividadeRec {
    ...
    public static function validateRequestCad($rawRequest, $edit = false) {
        ...
        $controlValidation->addValidator(new StringNotEmptyValidator("ciente", "O ARQUIVO PDF deve ser informado."));
    ...
    public static function filtroBasico($params) {
        ...
        if($grupo != "Apoio") {
            $strWhere .= " and tbatividade.para ='".$usuarioLogado['nome']."'";
            $strWhere .= ($ordem == 1) ? (" and (tbatividade.data like '{$atividade}%')") : ("");
            $strWhere .= ($ordem == 2) ? (" and (tbatividade.de like '{$atividade}%')") : ("");
            ...
            $strWhere .= " and (tbatividade.para ='".$usuarioLogado['nome']."' or tbatividade.para = '".$grupo."')";
            $strWhere .= ($ordem == 1) ? (" and (tbatividade.data like '{$atividade}%')") : ("");
            $strWhere .= ($ordem == 2) ? (" and (tbatividade.de like '{$atividade}%')") : ("");
            $strWhere .= ($ordem == 3) ? (" and (tbatividade.tipoAtividade like '{$atividade}%')") : ("");
            $strWhere .= ($ordem == 4) ? (" and (tbatividade.solicitacao like '%{$atividade}%')") : ("");
