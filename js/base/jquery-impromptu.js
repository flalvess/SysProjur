...
jQuery.extend({	
	ImpromptuDefaults: { prefix:'jqi', buttons:{ Ok:true }, loaded:function(){}, submit:function(){return true;}, callback:function(){}, opacity:0, zIndex: 999, overlayspeed:'slow', promptspeed:'slow', show:'fadeIn', focus:0, useiframe:false, bottom:"0", persistent: false },
	...
	ImpromptuClose: function(){
		jQuery('#'+ jQuery.ImpromptuCurrentPrefix +'box').fadeOut('slow',function(){ jQuery(this).remove(); });
	},
	ImpromptuCurrentPrefix: 'jqi',
	prompt: function(m,o,v){
		...
		var w = jQuery(window);
			...
			if (v== false) {
				msgbox += '<div class="'+ o.prefix +' ok" id="'+ o.prefix +'"><div class="'+ o.prefix +'container"><div class="'+ o.prefix +'close">X</div><div id="'+ o.prefix +'states"></div>';		
				msgbox += '</div></div></div>';
				
			}else{
			
				msgbox += '<div class="'+ o.prefix +' erro" id="'+ o.prefix +'"><div class="'+ o.prefix +'container"><div class="'+ o.prefix +'close">X</div><div id="'+ o.prefix +'states"></div>';		
				msgbox += '</div></div></div>';
			}			
		...
		var ie6scroll = function(){ 
			jqib.css({ bottom: w.scrollBottom() }); 
		};
		...
		var positionPrompt = function(){
			jqib.css({ position: (ie6)? "absolute" : "fixed", height: w.height(), width: "100%", bottom: (ie6)? w.scrollBottom():0, left: 0, right: 0, bottom: 0 });
			jqif.css({ position: "absolute", height: w.height(), width: "100%", bottom: 0, left: 0, right: 0, bottom: 0 });
			jqi.css({ position: "absolute", bottom: o.bottom, right: "0", marginLeft: ((jqi.outerWidth()/2)*-1) });					
		};
		...
		setTimeout(function() {removePrompt()},5000);
